...
export default function NodeDetailsPage() {
  const { enabledSourceIds, toggleSource, enableAllSources, showActiveOnly, setShowActiveOnly, activeHours, selectedNode } = useDataContext()
  ...
  return (
    <div className="node-details-page">
      <aside className="node-details-sidebar">
        ...
        <div className="source-filter" ref={dropdownRef}>
          ...
          >
            ...
            <span className="source-filter-chevron">{showSourceDropdown ? '^' : 'v'}</span>
          </button>
        ...
        {isLoading ? (
          ...
          <div className="empty-state">
            <div className="empty-state-icon">node</div>
            <p>No nodes found</p>
      ...
      <main className="node-details-content">
        {selectedNode ? (
          <NodeDetailsPanel node={selectedNode} />
        ) : (
          <div className="node-details-placeholder">
            <div className="placeholder-icon">i</div>
            <h2>Select a Node</h2>
            <p>Click on a node from the list to view detailed information and telemetry data.</p>
          </div>
        )}
      </main>
    </div>
